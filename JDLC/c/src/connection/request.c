...
struct request make_request(enum Op op, struct properties props, const char *restrict body)
{
    ...
    return (struct request) {.op = op, .props = props, .body = copy};
}
...
static size_t callback(void *contents, size_t size, size_t n, void *buffer)
{
    ...
    struct response *res = (struct response *) buffer;
...
static inline void prepare(CURL *handle, const char *url, const struct curl_slist *headers, struct request req, struct response *res)
{
